 ...
 *
 * $Id: file.c,v 1.3 2008/02/25 08:15:32 jgao Exp $
 */
#if 1
...
static int fgetpos(FILE * iofile,fpos_t *save_position)
{
	*save_position=(fpos_t) ftell(iofile);
	if(*save_position <0)
		return -1;
	return 0;
}

static int fsetpos(FILE * iofile, fpos_t *save_position)
{
	int ret;

	ret=(int) fseek(iofile,*save_position,SEEK_SET);
...
//  rewind(file->iofile);
  if(fseek(file->iofile, 0, SEEK_SET)== -1)
	  return -1;
			  ...
			  char const *path)
{
  ...
  file->iofile  = iofile;
  file->mode    = mode;
  file->path    = path ? strdup_id(path) : 0;
...
struct id3_file *id3_file_open(char const *path, enum id3_file_mode mode)
{
  ...
  assert(path);
  iofile = fopen(path, (mode == ID3_FILE_MODE_READWRITE) ? "rw" : "r");
  if (iofile == 0)
    return 0;
...
# if 1 || defined(HAVE_UNISTD_H)
  ...
  iofile = fdopen(fd, (mode == ID3_FILE_MODE_READWRITE) ? "rw" : "r");
  if (iofile == 0)
    return 0;
