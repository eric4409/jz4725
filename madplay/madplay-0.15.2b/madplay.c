 ...
 *
 * $Id: madplay.c,v 1.3 2008/04/08 04:17:38 jgao Exp $
 */
 ...
 */
static
void get_options(struct player *player)
{
  int opt, index;
  int ttyset = 0, preamp = 0;
...
//    case 's':
      player->global_start = get_time("00:00:00.000", 0, _("start time"));
      player->options |= PLAYER_OPTION_SKIP;
...
# endif
	...
	/* run the player */
	if (player_run(&player, 1, mp3name) == -1)
		result = 4;
