 ...
 *
 * $Id: player.c,v 1.3 2008/04/08 04:16:42 jgao Exp $
 */
...
//    printf("111--%d\n",stream->next_frame);
  ...
  do {
	  len = fread(input->data + input->length,1,
	       MPEG_BUFSZ - input->length,input->fd);
//	  len = fread(input->data,1,
//	       MPEG_BUFSZ,input->fd);
  }
  while (len == -1 && errno == EINTR);
//  printf("len:%d input len:%d\n",len,input->length);
  if (len == -1) {
    error("input", ":read");
		 ...
		 struct mad_header const *header, char const *label, int now)
{
  ...
  if (seconds != stats->nsecs || !on_same_line || now) {
    ...
    if (label || stats->label) {
      printf("%s %s", time_str, label ? label : stats->label);
	...
	case (MAD_FLAG_MS_STEREO | MAD_FLAG_I_STEREO):
      ...
      printf("%s\b\b\b\b\b\b\b\b\b",time_str);
#if 0
      printf("%s Layer %s, %s%u kbps%s, %u Hz, %s%s, %s",
...
//  unsigned int nchannels, nsamples;
  ...
  {
//		printf("play len:%d\n",towrite);
	  ret=pcm_write((char *)(pcmbuf+(2 * pcmcount)-towrite),towrite);
	  towrite -=ret;
  }
...
//  char const *file = player->playlist.entries[player->playlist.current];
	char const *file = player->playlist.entries;
  int result;


    player->input.path = file;
    player->input.fd   = fopen(file,"r");

//    player->input.fd   = open(file,"rb");
...
//	    _("clipped sample") : _("clipped samples"));
  ...
  fclose(player->input.fd);
//  if (player->input.fd != STDIN_FILENO &&
//      fclose(player->input.fd) == -1 && result == 0) {
