...
#define __gpio_set_port(p)      (REG_GPIO_PXDATS(p))
#define __gpio_clear_port(p)	(REG_GPIO_PXDATC(p))
...
#define __gpio_mask_irq_port(n)  REG_GPIO_PXIMS(n)
#define __gpio_unmask_irq_port(n)  REG_GPIO_PXIMC(n)
#define __gpio_ack_irq_port(n)  REG_GPIO_PXFLGC(n)
...
#define __wdt_stop_clk() \
	(REG_WDT_TCSR = (REG_WDT_TCSR & ~(WDT_TCSR_EXT_EN | WDT_TCSR_RTC_EN | WDT_TCSR_PCK_EN)))
