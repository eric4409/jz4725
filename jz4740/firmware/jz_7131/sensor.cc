...
//	printf("Init BIOKEY ... fingerbuf=%p,NewFPReader=%d \n",FingerCacheBuf,gOptions.NewFPReader);//treckle
	fhdl=BIOKEY_INIT(fd_sensor, (WORD*)sizes, params, (BYTE*)FingerCacheBuf, gOptions.NewFPReader);	
//	printf("SPEED=%d,MThreshold=%d\n",IDENTIFYSPEED,gOptions.MThreshold);//treckle
	BIOKEY_MATCHINGPARAM(fhdl, IDENTIFYSPEED, gOptions.MThreshold);
//	printf("%d %d %d \n",gOptions.MaxNoiseThr, gOptions.MinMinutiae, gOptions.MaxTempLen);//treckle
	BIOKEY_SETNOISETHRESHOLD(fhdl, gOptions.MaxNoiseThr, gOptions.MinMinutiae, gOptions.MaxTempLen, 500);	
//	printf("finished setnoiset\n");//treckle
	return (int)fhdl;
}
