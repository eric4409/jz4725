...
try_again:
	...
	printf("read gc clock reg:%x\n",val);
	if( (val&0x3) != 0x3)
	{
		printf("gc303 set clock failed ......\n");
		i2c_ok = -1;
	}

	val = sensor_read_reg(GC_FRAME);
        printf("read gc frame reg:%x\n",val);

        if( val!= 0x89)
        {
                printf("gc303 set frame failed ......\n");
                i2c_ok = -1;
        }

        if(i2c_ok==0)
        {
                printf("Initialized sensor GC303 register OK\n");
                return 0;
        }
        else
        {
                ...
                {
                        printf("Initialize sensor GC303 register ERROR!\n");
                        return -1;
                }
        }
...
//	sensor_write_reg(SCTRB, 0x15);       // VsHsEn, HSYNC mode
	...
	if(i2c_ok==0)
	{
		printf("Initialized sensor hv7131 register OK\n");
		return 0;
	}
	else
	{
		...
		{	
			printf("Initialize sensor hv7131 register ERROR!\n");
			return -1;
		}
	}
}
...
int sensor_init(int left, int top, int width, int height, int FPReaderOpt)
{
	...
	printf("Device ID from CMOS GC0303: %2x\n",ret);
        if(ret == 0x11)
        {
        	m_sensor_opt |= 0x20;
                printf("Found GC303\n");
	...
	else if(ret==0x29)
	{
        ...
        printf("Device ID from CMOS GC0307: %2x\n",ret);
        if(ret == 0x99)
        {
                printf("Found GC307\n");
	...
	printf("Device ID from CMOS HV7131: %2x\n",ret);
	if(ret==0x02)
        {
                printf("Found HV7131\n");
