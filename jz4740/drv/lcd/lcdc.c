...
struct lcd_desc *lcd_frame_desc2;

struct lcd_desc *cur_lcd_frame_desc,*old_lcd_frame_desc;
...
u8* lcd_get_change_frame(void) {return (u8*)LCD_UNCACHED(old_lcd_frame_desc->buf);}
u8* lcd_get_change_cframe(void) {return (u8*)(old_lcd_frame_desc->buf + 0x80000000);}
u8* lcd_get_change_phyframe(void){/*printf("old_lcd_frame_desc->buf = %x\n",old_lcd_frame_desc->buf);*/return (u8*)old_lcd_frame_desc->buf;}

u8* lcd_get_current_frame(void) {return (u8*)LCD_UNCACHED(cur_lcd_frame_desc->buf);}
u8* lcd_get_current_cframe(void) {return (u8*)(cur_lcd_frame_desc->buf + 0x80000000);}
u8* lcd_get_current_phyframe(void){return (u8*)cur_lcd_frame_desc->buf;}
...
void lcd_flush_frame(int x,int y,int w,int h)
{
	...
	{
				for(j = 0; j < h; j++)
					memset((jzfb.cframe2[i] + (x + (j + y) * jzfb.w) * 4),0, w * 4);
	}
}
void lcd_clean_frame_other() 
{
	...
	for(i = 0; i < Frame_Num;i++)
	{
		if((unsigned int)(jzfb.cframe2[i]) != (unsigned int)lcd_get_current_cframe())
...
void lcd_change_frame(void){
	...
	//REG_LCD_DA0 = PHYS(cur_lcd_frame_desc);
	old_lcd_frame_desc++;
	if((unsigned int)old_lcd_frame_desc > (unsigned int)&lcd_frame_desc2[Frame_Num - 1])
...
static struct lcd_desc *lcd_frame_desc2_room;
#else
#ifdef STATIC_MALLOC
static u8 lcd_heap[4096 + (480 * 272 * 4 + 4095) / 4096 * 4096 * FRAME_NUM] __attribute__((aligned(4096))); // 4Kb align

//static struct lcd_desc lcd_frame_desc2_room[Frame_Num] __attribute__ ((aligned (32)));
static struct lcd_desc *lcd_frame_desc2_room; //treckle
...
static int fb_malloc(void)
{
	...
	#ifdef FRAME_BUFF_ADDRESS
	jzfb.cpal = (u8*) lcd_heap;
	
	jzfb.cframe = (u8 *)((u32)lcd_heap);
	printf("2frame buff addr = %x %d\n",jzfb.cframe,Frame_Num * needroom / 2 );
	// lcd when 3buf is 16 bit
	lcd_frame_desc2_room = (struct lcd_desc *)((((unsigned int)lcd_heap + Frame_Num * needroom) + 31) & (~31));
	#else
	...
	for(i = 0; i < Frame_Num;i++)  
	{
			jzfb.cframe2[i] = (u8 *)((u32)jzfb.cframe + needroom * i);
			if(!GetLcdInit())
				memset(jzfb.cframe2[i],0,needroom);
			
	}
	jzfb.pal = (u8 *)LCD_UNCACHED(jzfb.cpal);
	jzfb.frame = (u8 *)LCD_UNCACHED(jzfb.cframe);
	for(i = 0; i < Frame_Num;i++) 
		jzfb.frame2[i] = (u8 *)LCD_UNCACHED(jzfb.cframe2[i]);
...
static void lcd_descriptor_init(void)
{
	...
	lcd_frame_desc2 = (struct lcd_desc *)LCD_UNCACHED(lcd_frame_desc2_room);
	...
	for(i = 0;i < Frame_Num;i++)
	{
	  ...
	  frame_desc1->cmd	 = frm_size;
		frame_desc1++;
		//printf("1:%x %x %x\n",frame_desc1,PHYS(frame_desc1),jzfb.frame2[i]);
  }
...
static void lcd_reinit()
{
	...
	if(jzfb.bpp == 18)
		...
		t = jzfb.bpp;
	int needroom = ((jzfb.w * t + 7) >> 3) * jzfb.h;
	printf("need room = %d frame = 0x%x\n",needroom,jzfb.cframe2[0]);
	for(i = 0; i < Frame_Num;i++)  
	{
		memset(jzfb.cframe2[i],0,needroom);
...
static int old_lcd_bpp = 0;
void lcd_enter_frame()
{
	//convert 16 bpp for mplayer 
	old_lcd_bpp = jzfb.bpp; 
	jzfb.bpp = 16;
	Frame_Num = 3;
	lcd_reinit();
