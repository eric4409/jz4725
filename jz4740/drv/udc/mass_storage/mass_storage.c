...
#if 1
static u32 massBuf_ptr[UDC_FRAME_SIZE / 4] __attribute__ ((aligned (32)));
#define udc_mass_alloc()
#define udc_mass_free()
...
static unsigned int HandleReceiveData(unsigned handle,unsigned char *buf,unsigned int len)
{
	...
	PUDC_BUS pBus = (PUDC_BUS)handle;
	unsigned int ret;
	unsigned int sectors = len / g_sector_size;
...
static void DeinitThread()
{
	if(FInitThread)
	{
		#ifdef MUTI_THREAD
		unsigned char err;
		OSTaskDel(MASS_TASK_PRIO);
		OSSemDel(sem_buf0,OS_DEL_ALWAYS,&err);
		sem_buf0 = NULL;
		OSSemDel(sem_buf1,OS_DEL_ALWAYS,&err);
		sem_buf1 = NULL;
		OSSemDel(sem_mass,OS_DEL_ALWAYS,&err);
		sem_mass = NULL;
		OSSemDel(sem_device,OS_DEL_ALWAYS,&err);
		sem_device = NULL;
		udc_mass_free();
		#else
		udc_mass_free();
