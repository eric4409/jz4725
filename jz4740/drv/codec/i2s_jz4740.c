...
#undef PAGE_SIZE
#endif
#define QUEUE_MAX 0x4
#define PAGE_SIZE	(0x10000 / QUEUE_MAX)
...
static long old_mute=0;  
static long new_mute=0;  
static long old_hw_vol=0;  
static long new_hw_vol=0;  
static int flag_CDCCR=0;  
...
inline  void set_hw_volume(void)
{
	//set CDCCR1
	if(flag_CDCCR)
	{
		...
		{	
			i2s_codec_set_mute(new_mute);
			old_mute=new_mute;
		}

		if(old_hw_vol!=new_hw_vol)
		{
			i2s_codec_set_volume(new_hw_vol);
			old_hw_vol=new_hw_vol;
		} 
		flag_CDCCR=0;
	}
...
//	printf("codec driver set rate: %d\n",rate);
...
int pcm_close(void)
{
	...
	id = out_busy_queue.oldid;
	//printf("[ id:%d ; left:%d ; right:%d ]",id,save_last_samples[id].left,save_last_samples[id].right);
	out_dma_buf_data_count[id] = insertpcm(out_dma_buf_data_count[id], (unsigned char *)out_dma_buf[id]);
	/*if(out_dma_buf_data_count[id] != 0) {
				dma_start(dma, out_dma_pbuf[id], PHYADDR(AIC_DR),
				out_dma_buf_data_count[id]);
...
static void SetAudioVolume(void *p)
{
	get_volume=p;
}
...
//		jz_audio_reset();
	...
	case PCM_SET_VOLFUNCTION:
			SetAudioVolume((void *)arg);
			break;
	case PCM_GET_SAMPLE_MAX:
		   return ((32767 * jz_audio_factor) >> 8);	
		   break;	
	default:
...
static unsigned int Page_Size = PAGE_SIZE;
...
//	printf("pcm_write %x %x\n",count, PAGE_SIZE);
	...
	while(pcm_ioctl(PCM_GET_SPACE, 0)<count);	//treckle
	...
	}else
	{
		...
		copy_count = count / 32 * 32; // 32 size align
		if(copy_count > 0)
		{	
			if ((id = get_buffer_id(&out_empty_queue)) >= 0) {
...
int pcm_init()
{
	...
	new_hw_vol=0;
	old_hw_vol=0;
	pcm_ioctl(PCM_SET_VOL,0);	/* 0-3 */

	new_mute=0;
	old_mute=0;
	pcm_ioctl(PCM_SET_MUTE,0);	/* 0-1 */
