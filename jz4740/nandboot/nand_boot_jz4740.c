...
static int block_size = 2048*64;//131072;
...
static int nand_read_oob(int page_addr, uchar *buf, int size)
{
	...
	nand_read_buf(buf, size, bus_width);
        ...
        /* wait for device ready for 512 page */
        if (page_size == 512)
                __nand_dev_ready();
...
#define POWER_ON        (32*2+11)
#define LV373_LE        (32*1+17)
#define  REDLED         (32*2+10)
...
static void gpio_init(void)
{
	...
	/* Power On the Device */
	 __gpio_as_output(POWER_ON);
        ...
        /* power on device */
//        __gpio_clear_pin(POWER_ON);
      	__gpio_set_pin(POWER_ON);
...
serial_puts(" ******************* ");
	...
	*(volatile unsigned int *)(0xa0aaaaaa) = 0x55555555;
	...
	serial_puts_hex((unsigned int)CFG_NAND_UCOS_OFFS);
	...
	serial_puts_hex((unsigned int)uboot);
	serial_puts(" ...");
