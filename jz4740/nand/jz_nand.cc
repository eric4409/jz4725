...
#if DMA_ENABLE
static void setzero(void *target,unsigned int len)
{
	unsigned int zero;
	unsigned char err;
	unsigned int* pzero = (unsigned int *)(((unsigned int) &zero) | 0xa0000000);
	*pzero = 0;
    int ch = DMA_CHANNEL;
	...
	REG_DMAC_DSAR(ch) = PHYSADDR((unsigned long)pzero);       
...
static inline void nand_write_memcpy(void *target,void *source,unsigned int len)
{
	...
	REG_DMAC_DRSR(ch) = DMAC_DRSR_RS_AUTO;	        	
		...
		REG_DMAC_DCMD(ch) = DMAC_DCMD_SAI| DMAC_DCMD_DAI| DMAC_DCMD_SWDH_32 | DMAC_DCMD_DWDH_8|DMAC_DCMD_DS_16BYTE | DMAC_DCMD_TIE;
	#else
...
#if DMA_INTERRUPT_ENABLE
			REG_DMAC_DCMD(ch) = DMAC_DCMD_SAI |DMAC_DCMD_DAI | DMAC_DCMD_SWDH_8 |
												  DMAC_DCMD_DWDH_32|DMAC_DCMD_DS_16BYTE| DMAC_DCMD_TIE;
  #else
...
static inline int nand_read_oob(int page,nand_page_info_t *info)
{
   	...
   	unsigned char *ptr;
    	unsigned int i;
  	unsigned char d;
//	printf("nand send read page for oob =%d\n",page);//treckle
...
//			info->spare_yaffs[4] &= ~(1<<j);
	...
	JZ_NAND_DESELECT;
	OP_NAND_UNLOCK();
//	printf("finished reading page\n");//treckle
//	for(j=935;j<935+10;j++)
//		printf("data in jz data[%d]=%x\n",j,data[j]);	
	return ret;
}
		  ...
		  if(info->page_status == 0xff)
		  {
		      ...
		      }else
		      {
					  ...
					  //memset(ptr,0,JZSOC_ECC_BLOCK);
						//if(*mask & (1 << j) == 0) setzero(ptr,JZSOC_ECC_BLOCK);	
					  ptr += JZSOC_ECC_BLOCK;
					  par += PAR_SIZE;
...
#if 1
	ptr = (unsigned char *) info;
//	printf("page first byte of spare is 0x%x\n",*ptr);//treckle
	for (i = 0; i < NAND_OOB_SIZE; i++)
		NAND_IO_ADDR = *ptr++;
#endif
...
/* jz_nand_write_page_info write page info for yaffs */
int jz_nand_write_page_info (void *dev_id, unsigned int page, nand_page_info_t *info)
{
	int i;
	unsigned char *ptr, status;
//	unsigned char buf[sizeof(nand_page_info_t)];
	unsigned char buf[64];	//treckle
	nand_page_info_t *tinfo=(nand_page_info_t *)buf;

	OP_NAND_LOCK();
	JZ_NAND_SELECT;
	
	if(nand_read_oob(page,tinfo))
        {
                JZ_NAND_DESELECT;
                OP_NAND_UNLOCK();
                return -EIO;
	...
	/* copy the ecc data of nand */
	memcpy(info->ecc_field,tinfo->ecc_field,36);
	
//	printf("nand write oop page=%d\n",page);//treckle
	nand_send_writeaddr(page,CONFIG_SSFDC_NAND_PAGE_SIZE);	
	ptr = (unsigned char *) info;
//	printf("first byte of spare is 0x%x\n",*ptr);//treckle
	...
	{
		OP_NAND_UNLOCK();
	//	printf("finished write oob in jz\n");//treckle
		return 0;
	}
 write_error:
	OP_NAND_UNLOCK();
...
#endif
#if 0
int jz_desc_readpage(void *dev_id, int page, unsigned char *data, nand_page_info_t *info,unsigned int mask)
{
	int i,j,ret = 0;
