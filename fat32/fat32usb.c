...
static FATFS file_system_usb;

int fatfile_Setup_usb(void)
{
	FRESULT err;
        if(USB_DetectStatus()!=0)
		return -1;

	err = f_mount(2, &file_system_usb);
	if(err != FR_OK)
		return -1;
...
//	f_mkdir("1:newdir");
	fat_err = f_open(&file_handler, "2:foo.txt", FA_READ|FA_WRITE | FA_CREATE_ALWAYS);
	if(fat_err!=0)
	{
		printf("fat error\n");
		return -1;
...
int fatfile_test_usb(void)
{
	FRESULT fat_err;
	FIL file_handler;
	FILINFO finfo;

       uint8_t Name[20]="2:testfat.txt";
...
int fatfile_pic_usb(void)
{
	FRESULT fat_err;
	FIL file_handler;
	FILINFO finfo;

       uint8_t Name[20]="2:testfat.jpg";
#define BUFSIZE (1024*512)
        int i;
        uint8_t buffer[BUFSIZE];
        uint8_t buf[BUFSIZE];
...
//	printf("%s\n",buffer);
	...
	strcpy(Name,"2:newdir/testfat.jpg");
	f_unlink(Name);
	fat_err = f_open(&file_handler, Name, FA_WRITE|FA_CREATE_NEW);
	if(fat_err!=0)
	{
...
int fatfile_attlog_usb(void)
{
        FRESULT fat_err;
        FIL file_handler;
        FILINFO finfo;

       uint8_t Name[20]="2:fatatt.dat";
#define BUFSIZE (1024)
        int i;
