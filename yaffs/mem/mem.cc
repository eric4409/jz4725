...
u8_t *mems=NULL;//treckle
int  memsize=0;//treckle
...
void mem_free(void *rmem)
{
    ...
    {
	printf("the address of rmem invalid\n");//treckle
...
//    printf("realloc malloc used mem is %d\n",newsize);//treckle
    sys_sem_signal();
    return rmem;
}
/*
...
//	printf(">>>>>>size of malloc =%d,  %d\n",size, size%4);//treckle
    /* Expand the size of the allocated memory region so that we can
    adjust for alignment. */
    //printf("oldsize:%d\t",size);//treckle
    ...
    for (ptr = (u8_t*)lfree - ram; ptr < MEM_SIZE; ptr = ((struct mem*) &ram[ptr])->next)
    {
		...
		memsize = (u8_t *)mem-mems;//treckle
		//printf("malloc mem %p\n",mem + SIZEOF_STRUCT_MEM);//treckle
		printf("malloc used mem is %d address is %p\n",memsize,(u8_t*)mem + SIZEOF_STRUCT_MEM);//treckle
    ...
    sys_sem_signal();
	printf("!!!!!! No Enough Memory For Malloc\n");//treckle
    return NULL;
}
