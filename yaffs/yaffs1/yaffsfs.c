...
int yaffs_open(const char *path, int oflag, ...)
{
	...
	if(handle >= 0)
	{
		...
		{
			/*  variable parameter --treckle */

			va_start(arg_ptr,oflag);
			mode=va_arg(arg_ptr,int);
			va_end(arg_ptr);
			if((mode&(S_IFMT|S_IFLNK|S_IFDIR|S_IFREG|S_IREAD|S_IWRITE))==0)
...
/* yaffs_flush is from yaffs close --treckle */
