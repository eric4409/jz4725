...
extern int nand_flash_program_buf(int blockpage, const unsigned char *data, const unsigned char *spare);
...
*/	//treckle
	...
	{
        	T(YAFFS_TRACE_WRITE,
                  (TSTR("Writing chunk %d with spare" TENDSTR), chunkInNAND));
//                   spare->objectId, spare->chunkId));
	}

	err = nand_flash_program_buf(chunkInNAND,(const unsigned char *)pdata,(const unsigned char *)spare);
...
//	if(blockNumber < dev->startBlock || blockNumber >= dev->endBlock)
	/* dev->endBlock is valid block, so we could allow to erase and  use it, is it right?  --treckle*/
	if(blockNumber < dev->startBlock || blockNumber > dev->endBlock) 
	{
		return YAFFS_FAIL;		
	}
