...
static int yaffs_GetTagsFromSpare(yaffs_Device *dev, yaffs_Spare *sparePtr,yaffs_Tags *tagsPtr);
...
static void yaffs_RetireBlock(yaffs_Device *dev,int blockInNAND)
{
	...
	/* For the nand flash K9G8G08U0A --treckle */
//	yaffs_WriteChunkToNAND(dev, blockInNAND * dev->nChunksPerBlock + dev->nChunksPerBlock - 1, NULL , &spare);
		...
		else if(level == 0)
		{
			...
			for(i = YAFFS_NTNODES_LEVEL0 -1; i >=0; i--)
			{
			    if(tn->level0[i])
		    	{
					...
					theChunk =  (tn->level0[i] << in->myDev->chunkGroupBits);
					T(YAFFS_TRACE_DELETION,(TSTR("soft delete tch %d cgb %d chunk %d" TENDSTR)						,tn->level0[i],in->myDev->chunkGroupBits,theChunk));
		...
		if(bi->blockState == YAFFS_BLOCK_STATE_EMPTY)
		{
			...
			dev->nErasedBlocks--;			
                        ...
                        T(YAFFS_TRACE_ALLOCATE,
                          (TSTR("Allocated block %d,  %d left" TENDSTR),
                           dev->allocationBlockFinder,dev->nErasedBlocks));
	...
	if(tags)
	{
		...
		if(yaffs_ReadChunkFromNAND(dev,chunkInNAND,NULL,&spare,1) == YAFFS_OK)
		{
			*chunkDeleted = (yaffs_CountBits(spare.pageStatus) < 7) ? 1 : 0;
			yaffs_GetTagsFromSpare(dev,&spare,tags);
#if 0
			if(yaffs_GetTagsFromSpare(dev,&spare,tags)<0)
			{
                                /* delete the chunk for the tags ecc error --treckle*/
				#ifdef POWERPROTECT	
				yaffs_DeleteChunk(dev,chunkInNAND,1);
                                printf("Unrecovered error occured on chunkInNAND %d\n",chunkInNAND);
				return YAFFS_FAIL;
				#endif
			}
#endif
...
static void yaffs_DeleteChunk(yaffs_Device *dev,int chunkId,int markNAND)
{
	...
	//	printf("*** yafss deleted the page No. %d\n",chunkId);//treckle
...
//	newTags.unusedStuff = 0xFFFFFFFF;
	newTags.unusedStuff = YAFFS_UNUSEDSTUFF;//treckle
...
int yaffs_UpdateObjectHeader(yaffs_Object *in,const char *name, int force)
{
    ...
    if(!in->fake || force)
    {
		...
		newTags.byteCount =   YAFFS_MAX_BYTECOUNT;
	//	newTags.byteCount =   0xFFFFFFFF;
	//	newTags.unusedStuff = 0xFFFFFFFF;
		newTags.unusedStuff = YAFFS_UNUSEDSTUFF;
		yaffs_CalcTagsECC(&newTags);
...
static int yaffs_IsBlockBad(yaffs_Device *dev, int blk)
{
	...
	/* for nand flash K9G8G08U0A  --treckle*/
//	yaffs_ReadChunkFromNAND(dev,blk * dev->nChunksPerBlock+dev->nChunksPerBlock-1,NULL,&spare,1); 
#if 1
...
static int yaffs_Scan(yaffs_Device *dev)
{
	...
	for(blk = dev->internalStartBlock; blk <= dev->internalEndBlock; blk++)
	{
		...
		for(c = 0; c < dev->nChunksPerBlock && state == YAFFS_BLOCK_STATE_SCANNING; c++)
		{
...
//			// This block looks ok, now what's in this chunk?
			yaffs_GetTagsFromSpare(dev,&spare,&tags);
                        ...
                        // This block looks ok, now what's in this chunk?
                        if( (yaffs_GetTagsFromSpare(dev,&spare,&tags)<0)&&
			yaffs_CountBits(spare.pageStatus)>=6)
                        {
                                /* delete the chunk for the tags ecc error --treckle*/
                                #ifdef POWERPROTECT
                                yaffs_DeleteChunk(dev,chunk,1);
                                printf("Unrecovered error occured on chunk %d\n",chunk);
                                continue;
                                #endif
                        }
#endif
