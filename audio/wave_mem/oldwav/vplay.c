...
#define DEFAULT_DSP_SPEED 	11025
...
//      wp->sub_chunk == FMT && wp->data_chunk == DATA) {	//treckle
   if(1){
    ...
    mychunk = wp->data_chunk;
    printf("sameplesize:%d dsp_speed:%d, count:%d\n",samplesize,dsp_speed,count);
    return 0;
  }
  return -1;
}
...
//	write (fd, &wh, sizeof(WaveHeader));
  	if (yaffs_write (fd,&wh,sizeof(WaveHeader)) != sizeof(WaveHeader)) {
...
//unsigned char recram[1024 * 1024]; //treckle  for recod
unsigned char recram[1];
 ...
 if (!quiet_mode) {
  ...
  if (direction == PLAY) {
	  ...
	  while (count) {
		  ...
		  if ( yaffs_read(myfile,audiobuf,c) > 0 ) {
			printf("read audio success from file\n");//treckle
...
//	 	 printf("read daata--%x-%x-%x-%x!\n",recram[recCount-4],recram[recCount-3],recram[recCount-2],recram[recCount-1]);
	 ...
	 wavfile = yaffs_open("wavenew.wav",O_RDWR);
	 get_wav_set();
	 start_wave(wavfile,recCount);
	 
	 if (yaffs_write (wavfile,recram,recCount) != recCount) {
		 printf("can not write to File %s!!",fd);
...
void record_play(char *name,int dir)
{
  ...
  if(dir==1)
	  ...
	  direction = RECORD; 
  if (direction == PLAY) {
 	  myfile = yaffs_open(name,O_RDWR);
	  printf("file open %x\r\n",myfile); 
	  if (myfile>=0) {
		  /* read SND-header */
		  yaffs_read(myfile,audiobuf,sizeof(WaveHeader));
		  /* read bytes for WAVE-header */
		  if (test_wavefile (audiobuf) >= 0)//treckle
	//	  if (1)
		  {
			printf("will recplay in test wavfile...\n");//treckle
...
int PlayWavFile(char *wavname)
{
	IS_WRITE_PCM = 1;
	record_play(wavname,IS_WRITE_PCM);
	return 0;
}
