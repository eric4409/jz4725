...
//void recplay (int fd, int loaded, u_long count, int rtype, char *name) //treckle
void recplay (int fd, int loaded, u_long count, int rtype, char *buf)
{
  int l, real_l;
  u_long c;
  char one_chn = 0;
...
//		  fmt_rec_table[rtype].what); //treckle
	 if (samplesize != 8)
...
void record_play(char *name,int dir)
{
  ...
  if (direction == PLAY) {
 	  myfile = FS_FOpen(name,"r");  //treckle
	  printf("file open %x\r\n",myfile); 
	  if (myfile) {
		  /* read SND-header */
//		  FS_FRead(audiobuf,1,sizeof(WaveHeader),myfile);//treckle
		  memcpy(audiobuf,buf,sizeof(WaveHeader));
			printf("testing wav\n");
		  /* read bytes for WAVE-header */
		  if (test_wavefile (audiobuf) >= 0)
			  recplay (fd,0, count, WAVE_FMT, buf+sizeof(WaveHeader));
//			  recplay (myfile,0, count, WAVE_FMT, name); //treckle
		  else {
			  /* should be raw data */
			  init_raw_data();
			  count = calc_count();
...
//	  recplay (myfile, sizeof(WaveHeader), count, record_type, name); //treckle
